...
- 🧩 **系统设置新增 `.env` 导入导出**（Issue #754）— 新增 `GET /api/v1/system/config/env-export` 与 `POST /api/v1/system/config/env-import`，支持在设置页一键备份/恢复 `.env`；导入后可自动触发运行时重载，便于桌面端（含 macOS DMG 升级）迁移配置。
